...
export function ActivityForm({ managers }: { managers: Manager[] }) {
  ...
  async function handleSubmit(e: React.FormEvent<HTMLFormElement>) {
    ...
    const body = {
      ...
      comanagerIds: selectedComanagers,
      qrCodeUrl,
    };
            ...
            >
              ...
              {managers.map((m) => (
                <option key={m.id} value={m.id}>
                  {m.name} ({m.email}){m.intern ? " [Intern]" : ""}
                </option>
              ))}
            </select>
          </div>
              ...
              {managers.map((m) => (
                <label key={m.id} className="flex items-center gap-2 text-sm">
                  <input
                    type="checkbox"
                    value={m.id}
                    checked={selectedComanagers.includes(m.id)}
                    onChange={(e) => {
                      if (e.target.checked) {
                        setSelectedComanagers((prev) => [...prev, m.id]);
                      } else {
